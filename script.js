...
function renderStatistics() {
  ...
  const averageMl = oils.length > 0 ? Math.round(totalMl / oils.length) : 0;
  ...
  const avgElem = document.getElementById('averageOilMl');
  if (avgElem) avgElem.textContent = averageMl.toLocaleString() + ' мл';
}
