 ...
 *
 * A compact button in the top right corner toggles between light and dark
 * modes. When clicked the `dark` class is toggled on the root html
 * element and a corresponding icon (moon for light mode, sun for dark
 * mode) is swapped. The preference is persisted in localStorage so
 * that the chosen theme sticks on subsequent visits.
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    button.addEventListener('click', () => {
        ...
        if (icon) {
            icon.setAttribute('data-feather', nowDark ? 'sun' : 'moon');
            feather.replace();
        }
    });
});
